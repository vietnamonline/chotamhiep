        ...
        <table style="width: 40%;margin-left:auto;" class="text-right sm-padding small strong">
	        <tbody>
		        <tr>
		            ...
		            <td>{{ single_price($order->orderDetails->where('seller_id', $user_id)->sum('price')) }}</td>
		        </tr>
		        <tr>
		            <th class="gry-color text-left">Shipping Cost</td>
		            <td>{{ single_price($order->orderDetails->where('seller_id', $user_id)->sum('shipping_cost')) }}</td>
		        </tr>
		        <tr class="border-bottom">
		            <th class="gry-color text-left">Total Tax</td>
		            <td>{{ single_price($order->orderDetails->where('seller_id', $user_id)->sum('tax')) }}</td>
		        </tr>
		        <tr>
		            <th class="text-left strong">Grand Total</td>
		            <td>{{ single_price($order->orderDetails->where('seller_id', $user_id)->sum('price') + $order->orderDetails->where('seller_id', $user_id)->sum('shipping_cost') + $order->orderDetails->where('seller_id', $user_id)->sum('tax')) }}</td>
		        </tr>
	        </tbody>
	    </table>
    </div>
